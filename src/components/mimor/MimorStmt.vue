...
onMounted(() => {
  ...
  const effect = mimor.router.findEffect(node.tag)
  if (!effect) return
...
<template>
  ...
  <component
    v-else-if="mimor.router.findCard(node.tag)"
    :is="mimor.router.findCard(node.tag)"
    :mimor="mimor"
    :element="node"
  />
  <div v-else-if="mimor.router.findEffect(node.tag)"></div>
  <div v-else>
